...
# Dependencies required by tools loaded dynamically with runpy.run_path().
# These MUST be imported by the launcher itself so PyInstaller includes them
# inside DucTool.exe. Merely importing them from a downloaded tool is not
# enough for PyInstaller's static analysis.
...
import texttable  # noqa: F401
import psutil  # noqa: F401
from Cryptodome.Cipher import AES as _Py7zrAES  # noqa: F401

# ============================================================
# TỰ SAO CHÉP DUCTOOL.EXE VỀ C:\duc VÀ DESKTOP
# ============================================================

SELF_COPY_DIR = Path(r"C:\duc")
SELF_COPY_EXE = SELF_COPY_DIR / "DucTool.exe"


def _get_windows_desktop_dir():
    """Lấy đúng Desktop hiện tại, kể cả khi Desktop được chuyển sang OneDrive."""
    if os.name == "nt":
        try:
            import ctypes
    ...
    # Fallback cho trường hợp API Windows không lấy được đường dẫn.
    return Path.home() / "Desktop"


def _same_file_content(path_a, path_b):
    """So sánh nhanh trước, chỉ tính SHA-256 khi kích thước bằng nhau."""
    try:
        path_a = Path(path_a)
        path_b = Path(path_b)
...
def _copy_current_exe_to(source, target):
    """Copy EXE hiện tại tới target theo kiểu thay thế an toàn."""
    target = Path(target)
...
def copy_ductool_to_c_drive():
    r"""
    Khi chạy bản PyInstaller, tự sao chép chính DucTool.exe tới:
      - C:\duc\DucTool.exe
      - Desktop\DucTool.exe

    Nếu một bản đích đã giống hệt bản đang chạy thì không copy lại.
    Nếu đang chạy chính file ở một vị trí đích thì bỏ qua vị trí đó.
    Lỗi ở một vị trí không được phép làm launcher ngừng chạy hoặc cản vị trí còn lại.
    """
    if not getattr(sys, "frozen", False):
        return

    source = Path(sys.executable).resolve()
    desktop_exe = _get_windows_desktop_dir() / "DucTool.exe"

    targets = [
        (SELF_COPY_EXE, r"C:\duc\DucTool.exe"),
        (desktop_exe, str(desktop_exe)),
    ]

    for target, display_name in targets:
        try:
            _copy_current_exe_to(source, target)
        except Exception as exc:
            # Không làm app chết nếu Windows đang khóa file đích hoặc thiếu quyền.
            try:
                print(f"Không thể sao chép DucTool.exe tới {display_name}: {exc}")
            except Exception:
                pass
...
def _print_tool_banner_fallback():
    """Fallback cho tool mới: hiển thị banner trước khi chạy tool.

    Tool có clear màn hình nên tự gọi banner của chính tool để banner luôn ở trên đầu.
    """
    try:
        print(LAUNCHER_DUC_TOOL_BANNER, flush=True)
        print(flush=True)
    except Exception:
        pass
...
def verify_py7zr_runtime():
    """Verify that the frozen EXE can really create and extract a .7z file."""
    try:
        with tempfile.TemporaryDirectory(prefix="ductool_py7zr_test_") as temp_dir:
            temp_dir = Path(temp_dir)
            source = temp_dir / "source.txt"
            archive_path = temp_dir / "test.7z"
            extract_dir = temp_dir / "out"

            payload = "DucTool py7zr runtime OK - 2026".encode("utf-8")
            ...
            extracted = extract_dir / "source.txt"
            if not extracted.exists() or extracted.read_bytes() != payload:
                raise RuntimeError("Nội dung sau giải nén không khớp.")
...
def verify_7zr_runtime():
    """Verify that official standalone 7zr.exe is bundled in the frozen EXE."""
    try:
        candidates = []
        ...
        candidates.append(Path(__file__).resolve().parent / "assets" / "7zr.exe")

        extractor = next((item for item in candidates if item.exists()), None)
        if extractor is None:
            raise FileNotFoundError("7zr.exe is not bundled")
...
if __name__ == "__main__":
    if len(sys.argv) >= 2 and sys.argv[1] == "--verify-py7zr":
        raise SystemExit(verify_py7zr_runtime())

    if len(sys.argv) >= 2 and sys.argv[1] == "--verify-7zr":
        raise SystemExit(verify_7zr_runtime())

    if len(sys.argv) >= 3 and sys.argv[1] == "--run-tool":
        raise SystemExit(run_downloaded_tool(sys.argv[2]))

    # Chỉ tự copy ở tiến trình launcher chính; tiến trình chạy tool con không copy lại.
    copy_ductool_to_c_drive()
